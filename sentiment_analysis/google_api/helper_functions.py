...
def resize_image_using_service(service_url, image_url, size=800):
    http_obj = httplib2.Http()
    b64Url = base64.b64encode(image_url)
    #print image_url, image_url
    payload = "?size={s}&urlBase64={u}".format(s=size, u=b64Url)

    resp, content = http_obj.request(
        uri=service_url + payload,
        method='GET'
    )
...
def get_storage_bucket(bucket_name, create_new=True):
    req = get_google_client("storage").buckets().get(bucket=bucket_name)
    try:
        resp = req.execute()
        return resp
    except errors.HttpError, e:
        if create_new:
            req = get_google_client("storage").buckets().insert(bucket=bucket_name)
            resp = req.execute()
            return resp
